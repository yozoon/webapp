...
if($USER["lesen"]){ //Mindestens Leserechte werden benötigt um etwas angezeigt zu bekommen.
	...
	?>
		<button data-toggle="collapse" data-target="#einst_rw" class="collapsed" style="background-color:rgba(0,0,0,0);border:0px;"><h5 class="fa" >Personen mit mindestens Leserechten bei diesen Einsatz:</h5></button>
		<div id="einst_rw" class="collapse">
			...
			$admin_users->execute($admin_users->errorInfo());
		...
		<div id="allsettings" class="border-top mt-4">
			<button data-toggle="collapse" data-target="#einst_ber" class="collapsed" style="background-color:rgba(0,0,0,0);border:0px;"><h5 class="mt-4 mb-4 fa">Organisationsberechtigungen ändern:</h5></button>
			...
			<div id="einsatz_organisationen" class="border-top mt-4">
				<button data-toggle="collapse" data-target="#einst_bet" class="collapsed" style="background-color:rgba(0,0,0,0);border:0px;"><h5 class="mt-4 mb-4 fa">Beteiligte Organisationen:</h5></button>
				<div id="einst_bet" class="collapse">
					<div class="d-flex align-items-end flex-column mb-4">
						<a href="pdf/einsatzberichtpdf.php?eb_organisationen=1" target="_blank" class="" data-toggle="tooltip" data-placement="center" title="Liste der beteiligten Organisationen drucken"><i class="material-icons">print</i></a>
					</div>
					...
					<?php
						...
						foreach($oids_el as $oid_t){ 
							if($oid_t != "DEV"){
								...
								if($verstecken == "false" || $verstecken == $_SESSION["etrax"]["OID"]){ // Versteckt Organisationseinträge, wenn diese entsprechend markiert sind.
									...
									<div class="<?php echo $oid_t; ?>_bet_org">
										<?php if(!$temp_org){ ?>
										<button data-toggle="collapse" data-target="#<?php echo $oid_t; ?>_bet_org" class="collapsed col-12 text-left" style="background-color:rgba(0,0,0,0);border:0px;"><h6 class="" style="background-color:<?php echo $col_prim; ?>;padding:10px;border-radius:10px 0px 0px 10px;"><?php echo $oidname[$oid_t]['bezeichnung']; ?></h6></button>
										<?php } ?>
										<?php if($temp_org){ ?>
										<div class="form-group row mt-4 mb-3 ml-2 mr-0" style="background-color:<?php echo $col_prim; ?>;border-radius:10px 0px 0px 10px;">
											<button data-toggle="collapse" data-target="#<?php echo $oid_t; ?>_bet_org" class="collapsed col-11" style="background-color:rgba(0,0,0,0);border:0px;"><div class="col-sm-11"><input <?php echo $readonly; ?> class="mb-2 form-control-plaintext checkJSON border-bottom border-white border-top-0" name="<?php echo $oid_t; ?>_orgname" id="<?php echo $oid_t; ?>_orgname" type="text" value="<?php echo $org2show[$oid_t]["orgname"]; ?>" tabindex="<?php echo $x;$x++; ?>"></div></button>
										...
										<div class="bg-info p-4 text-white rounded" id="<?php echo $oid_t; ?>_del_confirm" class="collapse" style="display:none">Soll die Organisation wirklich gelöscht werden? <button type="button" class="btn btn-success delete_elorg_confirm m-2" data-id="<?php echo $oid_t; ?>">Ja</button><button type="button" class="btn btn-danger abbruch m-2" data-dismiss="modal">nein</button></div>
										<?php } else { ?>
										</div>
										<?php }?>
										<?php } ?>
										...
										<div id="<?php echo $oid_t; ?>_bet_org" class="collapse">
											<div class="form-group row ml-2">
												...
												<div class="col-sm-12 pt-4">
													<button class="btn btn-success eingabe org_speichern float-right" id="" data-oid_t="<?php echo $oid_t; ?>" data-new="<?php echo isset($org2show[$oid_t]["id"]) ? "update" : "append";?>" tabindex="<?php echo $x;$x++; ?>">Speichern</button>
												</div>
												<?php } ?>
											</div>
											<?php
											...
											foreach($elids as $elid_t){ 
												...
												<div class="form-group row ml-4 mr-1" id="<?php echo $elid_t; ?>_div" style="background-color:<?php echo $col_sec; ?>;padding:5px;border-radius:10px;">
													...
													<div class="col-sm-12 pt-4">
														<button class="btn btn-success eingabe el_speichern float-right" id="" data-elid_t="<?php echo $elid_t; ?>" data-oid_t="<?php echo isset($elid2show[$elid_t]["oid"]) ? $elid2show[$elid_t]["oid"] : "";?>" data-new="update" tabindex="">Speichern</button>
													</div>
													<?php } ?>
												</div>
										...
										<button  class="btn btn-success add_el ml-4" data-oid="<?php echo $oid_t; ?>" style="margin-top:15px;margin-bottom:15px;">Einsatzleiter hinzufügen</button>
										<?php } ?>
										</div>
									</div>
									<?php 
							...
							<div class="col-12 text-center">
								<button  class="btn btn-primary bearbeiten" id="einst_bet_neu">Organisation hinzufügen</button>
							</div>
			...
			<div id="einsatz_organisationen" class="border-top mt-4">
				<button data-toggle="collapse" data-target="#einst_dar" class="collapsed" style="background-color:rgba(0,0,0,0);border:0px;"><h5 class="mt-4 mb-4 fa">Voreinstellungen:</h5></button>
		...
		<script>
			...
			jQuery(function() {
				...
				let Neuen_Administrator_anlegen = function(that){
					...
					let newadminuserUID = that.attr("data-uid");
					let newadminuser = that.attr("data-name");
					let newadminorgname = that.attr("data-orgname");
					let newadminuserID = that.attr("data-dnr");
					let eid = sessionStorage.getItem("eid"); //EID des Einsatzes
					let eids = that.attr("data-eid");
					if(eids == "-1" || eids == ""){
						eids = eid;
					} else {
						eids = eids+","+eid;
				...
				jQuery("#einst_bet_neu").click(function(e){
					...
					'<div class="'+oid_t+'_bet_org" style="background-color:#aee571;border-radius:10px;">'+
						'<h6 class="" style="background-color:#7cb342;padding:10px;border-radius:10px 0px 0px 0px;"><input class="mb-2 form-control-plaintext checkJSON border-bottom border-white border-top-0" name="'+oid_t+'_orgname" id="'+oid_t+'_orgname" type="text" value="" placeholder="Organisationsbezeichnung"></h6>'+
						'<div class="form-group row m-2">'+
							...
							'</div>'+
							'<div class="col-sm-12 pt-4">'+
								'<button class="btn btn-success eingabe org_speichern float-right mb-3" id="" data-oid_t="'+oid_t+'" data-new="append" tabindex="">Speichern</button>'+
				...
				jQuery(".add_el").click(function(e){
					...
					'<div class="form-group row ml-4 mr-1" style="background-color:#aee571;border-radius:5px;">'+
						...
						'<label class="col-sm-10 col-form-label pl-4" for="'+id+'-el_verstecken">Diese Information nur für die eigene Organisation anzeigen</label>'+
							...
							'</div>'+
						'<div class="col-sm-12 pt-4">'+
								'<button class="btn btn-success eingabe el_speichern float-right mb-3" id="" data-elid_t="'+id+'" data-oid_t="'+oid+'" data-new="append" tabindex="">Speichern</button>'+
								'<button class="abbrechen btn btn-danger float-right mr-2 mb-3" data-dismiss="modal" tabindex="">Abbrechen</button>'+
							'</div>'+
					'</div>'+
						'');
				...
				let Organisationsinfos_gespeichern = function(){
					$(".settingsmodal .modal-body").load( "inc/einstellungen.php", {"notroot":2} );
