...
if($USER["lesen"]){ //Mindestens Leserechte werden benötigt um etwas angezeigt zu bekommen.
	...
	?>
				<form id="checkliste" method="post">
					<h3>Eintreffen am Berufungsort:</h3>
				...
				<?php if($USER["gleich"]){ ?>
					<button  class="btn btn-primary checklist-bearbeiten" type="button">Bearbeiten</button>
					<button class="btn btn-success eingabe" id="checklistesubmit" tabindex="15" type="button">Speichern</button>
					<button class="btn btn-danger eingabe abbrechen" tabindex="16" type="button">Abbrechen</button>
				<?php } ?>
				...
				//Verhindern der Eingabe von " und \ für JSON
					...
					$(".checklisteEinsatz").on("click", "#checklistesubmit", function() {
						...
						//für read_write_db
							...
							success: function () {
								//$(".checklisteEinsatz .modal-body").load( "inc/checkliste.php", {"notroot":2} );
								$("#checkliste input").attr("disabled",true);
								$("#zielpersonsubmit,.abbrechen,.eingabe").hide();
								$(".checklist-bearbeiten,.schliessen").show();
