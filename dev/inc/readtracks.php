...
foreach ($EinsatzGruppen as $key => $egroup) {
	...
	$user_in_group->execute($usertracks->errorInfo());
	...
	while ($uid_t = $user_in_group->fetch(PDO::FETCH_ASSOC)){
		...
		if(isset($userdata_arr[$uid_t])){
			echo "<br>Line 170: uid_t: ".$uid_t."<br>";
							...
							if($counterBOS){
								...
								$gpxtracks_temp .= '{
									...
									"properties": {
										...
										"img": "'.$track_icon.'",
										"beschreibung": "<b style=color:'.$trackcolor.'>Track mit '.$group_name.'</b><br>DNR: '.$DNR.'<br>BOS: '.$BOSnr.'<br><small>'.$tracktime.'</small>"
									},
									"geometry": {
