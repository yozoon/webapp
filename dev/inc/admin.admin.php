...
<?php  		// User Anzeigen
			...
			while ($res_mg = $db_admin->fetch(PDO::FETCH_ASSOC)){
				$data_user_json = json_decode(substr(string_decrypt($res_mg['data']), 1, -1));
				$admin_arr[] = array('UID' => $res_mg['UID'], 
									'OID'   => $res_mg['OID'], 
									'FID'   => $res_mg['FID'], 
									'name'   => isset($data_user_json->name) ? $data_user_json->name : "", 
			...
			if($n_user > 0) {
				...
				<li class="showadmin list-group-item list-group-item-action d-flex justify-content-start">
					<a href="#" data-oid="<?php echo($inhalt['OID']);?>" data-uid="<?php echo($inhalt['UID']);?>" data-name="<?php echo($inhalt['name']);?>" data-login="<?php echo($inhalt['login']);?>" data-email="<?php echo($inhalt['email']);?>" data-fid="<?php echo($inhalt['FID']);?>" data-eids="<?php echo $eidtemp;?>" data-eidstext="<?php echo $eidtext_lang;?>"><?php echo($inhalt['name']);?> | <?php echo $eidtext; ?></a>
				</li>
				<?php							
...
<div class="modal fade adminmodal" tabindex="-1" role="dialog" aria-labelledby="usermodalheader" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="settingsmodalheader" style="font-weight:bold;">Admin Details: </h5>
			</div>
			<div class="modal-body">
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<input disabled type="text" name="y_name" class="mb-2 form-control-plaintext y_admin_edit y_name checkJSON" id="y_name" placeholder="Vollständiger Name des Administrators" value=""></input>
					</div>
				</div>
				<div class="form-group row">
					<label  class="col-sm-3 col-form-label" for="y_loginname">Username</label>
					<div class="col-sm-9">
						<input disabled type="text" name="y_loginname" class="mb-2 form-control-plaintext y_admin_edit y_loginname checkJSON usernamecheckadmin" id="y_loginname" placeholder="Username des Administrators" value=""></input>
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<input disabled type="password" name="y_pwd" class="mb-2 form-control-plaintext y_admin_edit y_pwd checkJSON pwdcheckadmin" id="y_pwd" aria-describedby="pwdHelp1" placeholder="Feld leer lassen für keine Änderung" value=""></input>
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<input disabled type="password" name="y_repwd" class="mb-2 form-control-plaintext y_admin_edit y_repwd checkJSON repwdcheckadmin" id="y_repwd" aria-describedby="pwdHelp1" placeholder="Feld leer lassen für keine Änderung" value=""></input>
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<input disabled type="text" name="y_email" class="mb-2 form-control-plaintext y_admin_edit y_email checkJSON" id="y_email" placeholder="E-Mailadresse" value=""></input>
					</div>
				</div>
				<div class="form-group row">
				...
				<div class="mx-auto order-md-1 text-center mt-4">	
					<button type="button" class="btn btn-primary admin_modify" title="User Daten bearbeiten" data-toggle="tooltip" data-placement="bottom">Daten bearbeiten</button>
					<button type="button" class="btn btn-success abschliessen admin_modify_save" title="Änderungen speichern" data-toggle="tooltip" data-placement="bottom" style="display:none;" data-uid="">Änderungen speichern</button>
					<button type="button" class="btn btn-warning admin_modify_downgrade" title="User Administratorenberchtigung entziehen" data-toggle="tooltip" data-placement="bottom" data-uid=""><i class='material-icons text-dark'>cancel</i></button>
					<button type="button" class="btn btn-danger admin_modify_delete" title="Administrator inklusive User löschen" data-toggle="tooltip" data-placement="bottom" style="display:none;" data-uid=""><i class='material-icons text-white'>delete_forever</i></button>
				</div>
			</div>
		</div>
	</div>
