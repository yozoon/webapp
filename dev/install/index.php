...
<html lang="de">
  ...
  <body class="bg-light">
	...
	?>	
			...
			<ul>
				<?php $check_t = is_file( "../js/bootstrap.bundle.min.js") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">js/bootstrap.bundle.min.js</li>
				<?php $check_t = is_file( "../js/bootstrap.bundle.min.js.map") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">js/bootstrap.bundle.min.js.map</li>
			</ul>
			<h5 class="p-2 mt-3">jQuery</h5>
			<ul>
				<?php $check_t = is_file( "../js/jquery-2.2.3.min.js") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">js/jquery-2.2.3.min.js</li>
			</ul>
			<h5 class="p-2 mt-3">Trix Editor</h5>
			<ul>
				<?php $check_t = is_file( "../js/trix.js") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">js/trix.js</li>
				<?php $check_t = is_file( "../css/trix.css") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">css/trix.css</li>
			</ul>
			<h5 class="p-2 mt-3">TCPDF</h5>
			<ul>
				<?php $check_t = is_file( "../pdf/tcpdf.php") ? true : false; $install = $check_t ? $install : false;?>
				<li class="<?php echo $check_t ? "text-success" : "text-danger"; ?>">pdf/tcpdf.php</li>
			</ul>
			<?php if($install){ ?>
			<div class="mx-auto order-md-1 text-center mt-4">	
	...
	<h3 class="p-3 mb-2 bg-primary text-white rounded"><b>Schritt 3: </b>Anlegen eines zufälligen Schlüssels</h3>
			<input type="hidden" name="step" value="<?php echo $step+1;?>"</input>
			<?php
				$secretkey = generate_string($permitted_chars, 64);
				$secretiv = generate_string($permitted_chars, 28);
...
# Return 403 Forbidden header and show the content of the root home page';
				...
				$dateien = array("../gpx/.htaccess","../img/.htaccess","../inc/.htaccess","../pdf/.htaccess");
			...
			<h5 class="p-2 mt-3">Initialization Vector:</h5>
			<ul>
				<li class="text-success"><?php echo $secretiv; ?></li>
			</ul>
...
?>';
	...
	<?php
		...
		$setDB = $db->prepare("
			...
			CREATE TABLE `externeuser` (
			  `ID` int(11) NOT NULL,
			  `SID` int(11) NOT NULL,
			  `EID` int(11) NOT NULL,
			  `name` varchar(300) NOT NULL,
			  `dienstnummer` varchar(300) NOT NULL,
			...
			--
			-- Indizes für die Tabelle `externeuser`
			--
			ALTER TABLE `externeuser`
			  ADD PRIMARY KEY (`ID`);
			...
			--
			-- AUTO_INCREMENT für Tabelle `externeuser`
			--
			ALTER TABLE `externeuser`
			...
			INSERT INTO `workstations` (`TAN`, `SID`, `bezeichnung`) VALUES
			('1', 1, 'HOST');
	...
	<?php
		...
		//Userdaten erzeugen
			...
			$pwd = encryptdb($newpassword,"user","pwd");
