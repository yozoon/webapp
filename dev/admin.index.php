...
if ($userlevel < 8 && is_numeric($userlevel)) {
	/*function decryptsettings($string) {
		...
		// hash
		//$key = hash('sha256', $key);
		$key = hash('sha256', $GLOBALS['secretkey']);
		
		// iv - encrypt method AES-256-CBC expects 16 bytes - else you will get a warning
		$iv = substr(hash('sha256', $GLOBALS['secretiv']), 0, 16);
...
?>
	...
	<body id="admin_index" class="background">
		...
		?>
				...
				<div class="tab-content" id="adminTabContent">
					...
					<div class="tab-pane fade show active" id="organisation" role="tabpanel" aria-labelledby="organisation-tab">
						<?php require "inc/admin.organisation.php"; ?>
					</div>
					<!-- Organisationen anzeigen Ende -->

					<!-- Userverwaltung Anfang -->
					<div class="tab-pane fade" id="user" role="tabpanel" aria-labelledby="user-tab">
						<?php require "inc/admin.user.php"; ?>
					</div>
					<!-- Userverwaltung Ende -->
					...
					<div class="tab-pane fade" id="ressourcen" role="tabpanel" aria-labelledby="ressourcen-tab">
						<?php require "inc/admin.ressourcen.php"; ?>
					</div>
					...
					<?php if($userlevel <= 3 && is_numeric($userlevel)){ ?>
						<div class="tab-pane fade" id="admin" role="tabpanel" aria-labelledby="admin-tab">
							<?php require "inc/admin.admin.php"; ?>
						</div>
					<?php } ?>
		...
		<?php //Infozeile für die Entwicklung
				...
				if (is_file("inc/admin.info.php")) {
					include "inc/admin.info.php";
				}
				break;
			...
			case "import":
				...
				if ($iDel == 1) { //Bestehende Nutzer der Organisation löschen
					$org_del = $db->prepare("DELETE FROM user WHERE OID = '" . $iOrg . "' AND UID != '" . $_SESSION["etrax"]["UID"] . "' AND (FID = '10' OR FID = '8.1' OR FID = '8.2' OR FID = '8.3' OR FID = '8.4' OR FID = '8.5')"); // Dadurch werden Administratoren nicht gelöscht
					$org_del->execute($org_del->errorInfo());
				}
				foreach ($_POST["iImport"] as $i) {
					$UID = $iOrg . "-" . htmlspecialchars($_POST["i_2"][$i], ENT_QUOTES);
					//$UID_admin = encryptdb($iOrg."-".htmlspecialchars($_POST["i_2"][$i],ENT_QUOTES),"adminuser","UID");
					$name = htmlspecialchars($_POST["i_1"][$i], ENT_QUOTES);
					//$name_admin = encryptdb(htmlspecialchars($_POST["i_1"][$i],ENT_QUOTES),"adminuser","name");
					...
					$db_vorhanden = $db->prepare("SELECT UID FROM user WHERE UID = '" . $UID . "' AND OID = '" . $iOrg . "'");
					$db_vorhanden->execute($db_vorhanden->errorInfo());
					$uvorhanden = 0;
					...
					if ($uvorhanden > 0) { //es gibt den User schon --> Update
						if ($iPwUp == 1) {
							...
							$sql_query = $db->prepare("SELECT data, pwd FROM user WHERE `UID`='" . $UID . "'");
						...
						$update = $db->prepare("UPDATE user SET `UID`='" . $UID . "',`OID`='" . $iOrg . "',`data`='" . $encrypted . "',`username`='" . $username_sha256 . "'" . $pwd_up . " WHERE `UID`='" . $UID . "' AND OID = '" . $iOrg . "'");
						...
						//JSON erzeugen
						$encrypted = string_encrypt(json_encode($data, JSON_UNESCAPED_UNICODE));
						$insert = $db->prepare("INSERT INTO user (`UID`, `OID`, `data`, `username`, `pwd`, `FID`, `EID`) VALUES ('" . $UID . "','" . $iOrg . "','" . $encrypted . "','" . $username_sha256 . "','" . $pwd . "', '10', '-1')");
		...
		<script type="text/javascript" src="js/admin.diverse.js"></script>
		<script type="text/javascript" src="js/admin.admin.js"></script>
		<script type="text/javascript" src="js/admin.organisation.js"></script>
		<script type="text/javascript" src="js/admin.user.js"></script>
		<script type="text/javascript" src="js/admin.ressourcen.js"></script>
	</body>
