...
if(isset($_SESSION["etrax"]["UID"])) { //redirect auf einsatzstart.php wenn man bereits angemeldet ist
...
require "inc/startseitentexte.php"; //Textelemente auf der Startseite
require "inc/include.php"; //Textelemente auf der Startseite
include "verschluesseln.php";
...
if(isset($_POST["submited"])){//print_r($_POST);
	...
	$user_sql = $db->prepare("SELECT EID,OID,UID,FID,username,data,aktiveEID FROM user WHERE username LIKE '".hash("sha256",$postOID."-".$login,false)."'"); //Username wird direkt geprüft
	...
	while ($rowuser = $user_sql->fetch(PDO::FETCH_ASSOC)){
		...
		$org_sql = $db->prepare("SELECT Laufzeit FROM lizenzen WHERE OID LIKE '".$rowuser["OID"]."'");
		...
		//if($username == $usersalt[0] && $islicense){
...
?>
	...
	<div id="mama">
		...
		<footer class="landingfooter d-flex flex-column">
			...
			<ul class="landingfootercontainer nav d-flex justify-content-center">
				<li class="nav-item">
				<a class="nav-link about_login" href="#">Informationen zu eTrax | rescue</a>
				</li>
				<li class="nav-item">
			...
			<div class="modal-content">
				...
				<div id="login">
					...
					<form id="loginform" class="mb-4" action="<?php //echo $_SERVER['PHP_SELF']?>" method="post">
						...
						<select name="oid" id="oid" class="mb-2">
						...
						<input type="text" class="mb-2" size="24" maxlength="50" name="username" id="username" placeholder="Username" value="<?php if(isset($_POST["username"])){echo htmlspecialchars($_POST["username"], ENT_QUOTES);}?>" tabindex="1">
						
						<label for="password" class="sr-only">Passwort:</label>
						<input type="password" size="24" maxlength="50" name="password" id="password" tabindex="2" placeholder="Passwort" class="mb-2">
				...
				<div id="pwdreset">
					...
					<div class="text-info m-4 p-2 rounded">
						...
						<form id="resetform" class="m-4 text-center" action="<?php //echo $_SERVER['PHP_SELF']?>" method="post">
							...
							<input type="text" class="form-control" size="24" maxlength="50" name="email2reset" id="email" placeholder="E-mailadresse" value=""  required tabindex="1">
	...
	<!-- Impressum Ende -->
	<!-- eTrax Info -->
		<div class="modal fade about" tabindex="-1" role="dialog" aria-labelledby="info_h" aria-hidden="true">
			<div class="modal-dialog modal-xl">
				<div class="modal-content">
					<div class="modal-header">
						<h1 class="modal-title" id="info_h">Informationen zu eTrax | rescue</h1>
					</div>
					<div class="modal-body">
						<?php
						echo $text["info"];
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<h5 class="bg-success m-4 p-4 rounded" id="resetverarbeitet" style="display:none">Ihre Anfrage wurde verarbeitet. Bitte kontrollieren sie ihre Mailbox.</h5>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary btn-reload" data-dismiss="modal">Schliessen</button>
					</div>
	...
	<!-- eTrax Info Ende -->
		...
		if(isset($_POST["resetpwd"])){
			if(isset($_POST["email2reset"])){ ?>
				...
				$db_mitglieder = $db->prepare("SELECT * FROM user WHERE OID = '".$oid4reset."'");
				...
				while ($res_mg = $db_mitglieder->fetch(PDO::FETCH_ASSOC)){
					...
					if($email2reset == mb_strtolower($email_t, 'UTF-8')){
						...
						$update = $db->prepare("UPDATE user SET `pwdresetkey`='".$randomkey."' WHERE `UID`='".$uid_t."' AND OID = '".$oid4reset."'");
