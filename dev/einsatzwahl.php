...
require "inc/startseitentexte.php"; //Textelemente auf der Startseite - für Datenschutzinfo
...
$EID ? $neueEID = $EID + 1 : $neueEID = 1;
$imEinsatzCount = $EIDaktuell = 0;
//$dienstnummer = encryptdb($_SESSION["etrax"]["dienstnummer"],'personen_im_einsatz','dienstnummer');
//Einsatz wählen in dem User zugewiesen ist
$imEinsatz_query = $db->prepare("SELECT aktiveEID FROM user WHERE UID = '".$_SESSION["etrax"]["UID"]."'");
$imEinsatz_query->execute($imEinsatz_query->errorInfo());
while ($row = $imEinsatz_query->fetch(PDO::FETCH_ASSOC)){
	$einsatz_query = $db->prepare("SELECT EID,data FROM settings WHERE EID = '".$row["aktiveEID"]."'");
...
?>		
	...
	<body>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="neuereinsatzheader">Neuer Einsatz Nr:<?php echo $neueEID;?></h5>
					</div>
				...
				<?php //Anzeige der Einsätze
					...
					if(!isset($_POST['einsatz']) && $_SESSION["etrax"]["usertype"] == "administrator"){
							...
							if(in_array($OID_admin,$oids_t) || $OID_admin == "DEV" ){
								...
								if($einsatztyp === "einsatz"){
									...
									if($userlevel <= 3 && $userrechte <= 1){
										$delete_button ='<button class="e-delete btn btn-danger mt-4 mr-2" data-eid="'.$einsaetze["EID"].'">Löschen</button>';
									} else {
										$delete_button = "";
									}
										...
										<form class="einsatzauswahl" method="post" action="einsatz-start.php">
											...
											<div class="card mb-4 <?php echo $styling;?>">
												<div class="card-body">
													...
													<ul class="list-group">
														<li  class="list-group-item bg-success">Primärorganisation:</li>
														<li class="list-group-item"><?php echo $oids[$OID]; ?></li>
														<?php
														$i = 0;
														if(count($oids_t) > 1){
															foreach($oids_t as $oid_tc){
																echo $i == 0 ? '<li class="list-group-item bg-info">Beteiligte Organisationen:</li>' : "";
						...
						<div class="category card m-4 suchgebiete">
							<div class="card-header"><h3> Sie sind im Einsatz <?php echo $Enameaktuell; ?> angemeldet.</h3><h5>Angelegte Suchgebiete</h5></div>
							<div class="card-body">
							</div>
						</div>
					...
					<div class="modal-body">
						<form class="p-4">
							<div class="admin_field form-group row">
								<input type="hidden" class="x_uid" id="uid" value="<?php echo $_SESSION["etrax"]["UID"];?>">
								<input type="hidden" class="x_oid" id="oid" value="<?php echo $_SESSION["etrax"]["OID"];?>">
								<label for="name" class="col-md-3 col-form-label">Name:</label>
								<div class="col-md-9">
									<input class="form-control-plaintext x_user_edit x_name" readonly type="text" name="name" id="name" placeholder="Vollständiger Name des Users" value="<?php echo $_SESSION["etrax"]["name"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="dienstnummer" class="col-md-3 col-form-label">Dienstnummer:</label>
								<div class="col-md-9">
									<input class="form-control-plaintext x_dienstnummer" readonly type="text" name="dienstnummer" id="dienstnummer" placeholder="Dienstnummer" value="<?php echo $_SESSION["etrax"]["dienstnummer"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="typ" class="col-md-3 col-form-label">Typ:</label>
								<div class="col-md-9">
									<input class="form-control-plaintext x_typ" readonly type="text" name="typ" id="typ" placeholder="Typ" value="<?php echo array_key_exists($_SESSION["etrax"]["typ"],$fun_list) ? $fun_list[$_SESSION["etrax"]["typ"]] : "";?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="username" class="col-md-3 col-form-label">Username:</label>
								<div class="col-md-9">
									<input class="form-control-plaintext x_username" readonly type="text" name="username" id="username" placeholder="Username" value="<?php echo $_SESSION["etrax"]["username"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="pwd" class="col-md-3 col-form-label">Passwort:</label>
								<div class="col-md-9">
									<input class="form-control x_user_edit x_pwd pwdcheck" type="password" name="pwd" id="pwd" placeholder="Feld leer lassen für keine Änderung" value="" aria-describedby="pwdHelp">
							...
							<div class="admin_field form-group row">
								...
								<div class="col-md-9">
									<input class="form-control x_user_edit x_email" type="text" name="email" id="email" placeholder="E-Mailadresse" value="<?php echo $_SESSION["etrax"]["email"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">	
							...
							<div class="admin_field form-group row">
								...
								<div class="col-md-9">
									<input class="form-control x_user_edit x_bos" type="text" name="bos" id="bos" placeholder="BOS" value="<?php echo $_SESSION["etrax"]["bos"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								...
								<div class="col-md-9 input-group">
									<div class="input-group-prepend">
										<div class="input-group-text">+43</div>
									</div>
									<input class="form-control x_user_edit x_telefon" type="text" name="telefon" id="telefon" placeholder="Telefon" value="<?php echo $_SESSION["etrax"]["telefon"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="notfallkontakt" class="col-md-3 col-form-label">Notfallkontakt:</label>
								<div class="col-md-9">
									<input class="form-control x_user_edit x_notfallkontakt" notfallkontakte="text" name="notfallkontakt" id="notfallkontakt" placeholder="Notfallkontakt" value="<?php echo $_SESSION["etrax"]["notfallkontakt"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="notfallinfo" class="col-md-3 col-form-label">Notfallinfo:</label>
								<div class="col-md-9">
									<input class="form-control x_user_edit x_notfallinfo" type="text" name="notfallinfo" id="notfallinfo" placeholder="Notfallinfo" value="<?php echo $_SESSION["etrax"]["notfallinfo"];?>">
								</div>
							</div>
							<div class="admin_field form-group row">
								<label for="kommentar" class="col-md-3 col-form-label">Kommentar für Einsatzleiter:</label>
								<div class="col-md-9">
									<textarea class="form-control-plaintext x_kommentar" readonly type="text" name="kommentar" id="kommentar" placeholder="kommentar"><?php echo $_SESSION["etrax"]["kommentar"];?></textarea>
								</div>
							</div>
						</form>
					</div>
		...
		<?php } ?>
		<script>
			window.neueEID = <?php echo $neueEID;?>;
