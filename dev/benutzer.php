...
/*
function decryptsettings($string) {
	globals $secretkey, $secretiv;
	require "../../secure/secret.php";
    $output = false;
...
while ($roworg = $oidname_query->fetch(PDO::FETCH_ASSOC)) {
	...
	$function_list = json_decode($roworg["funktionen"], true);
}
$f_options = '';
if ($function_list != '') {
...
$f_select = '<select name="typ" class="usertyp">' . $f_options . '</select>';
...
<body id="benutzer" class="background">
	...
	<div class="modal fade changeuser" tabindex="-1" role="dialog" aria-labelledby="changeuserheader" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="changeuser"></h5>
				</div>
				<form id="changeUser" class="d-flex">
					<div class="modal-body">
						<div class="col-12">
							Im Einsatz als: <?php echo $f_select; ?>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary">Änderungen speicher</button>
		...
		<div class="userlist row mb-4 pb-4">
			<div id="memberlist" class="col-12 col-lg-4 p-2">
				<div class="bg-lightgray p-2">
					<h4 class="ui-widget-header btn btn-secondary w-100">User und Material</h4>
					<input class="btn btn-outline-primary bg-white color-black sorterinput listfilter mr-2" placeholder="filtern nach ..." data-target="#memberlist #members ul li" id="usersort" type="text"><i title="Liste neu laden" class="sync material-icons">backspace</i>
					...
					<ul id="members" class="list-group">
						...
						function trackingUser()
						{
							...
							$GPStracker = [];
							$query_tracker = $db->prepare("SELECT UID,nummer,altitude,timestamp_server,hdop FROM tracking WHERE EID LIKE " . $_SESSION["etrax"]["EID"] . " GROUP BY UID");
						...
						function MitgliederimEinsatz()
						{
							...
							$sql_query = $db->prepare("SELECT personen_im_einsatz FROM settings WHERE EID = " . $EID);
						...
						function MitgliederinallenEinsaetzen()
						{
							...
							$sql_query = $db->prepare("SELECT UID,aktiveEID FROM user WHERE aktiveEID NOT LIKE " . $EID . " AND aktiveEID NOT LIKE ''");
							$sql_query->execute($sql_query->errorInfo());
							while ($sql_json = $sql_query->fetch(PDO::FETCH_ASSOC)) {
						...
						$HFcount = $Hcount = $ELcount = 0;
						$User_in_Pause = $User_im_Einsatz = $User_im_Einsatz_UID = [];
					...
					<li class="btn btn-link font-weight-bold color-white bg-red mt-3">Material <?php echo $oidname[$_SESSION["etrax"]["OID"]]['kurzname']; ?></li>
					<?php
					$sql_ressourcen = $db->prepare("SELECT ID,OID,RID,data,typ,aktiveEID FROM ressourcen WHERE OID = '" . $_SESSION["etrax"]["OID"] . "'");
					$sql_ressourcen->execute($sql_ressourcen->errorInfo());
					while ($sql_json = $sql_ressourcen->fetch(PDO::FETCH_ASSOC)) {
						...
						if ($ressource[0]['name'] !== '') {
							...
							echo $ressource[0]['typ'] . ' ' . $ressource[0]['name'] . ' ' . $ressource[0]['kennung'];
							echo $_SESSION["etrax"]["USER"]["zuweisen"] ? "<i class='moveright material-icons ml-auto'>arrow_forward</i></li>\n" : "</li>\n";
						}
					}
					?>
			...
			<div id="imEinsatz" class="groupbox col-12 col-lg-4 p-2">
				<div class="bg-lightgray p-2">
					<h4 class="ui-widget-header btn bg-green color-white w-100">Im Einsatz <?php //echo 'HF:'.$HFcount.' - H:'.$Hcount.' - EL:'.$ELcount; 
																							?></h4>
					<input class="btn btn-outline-primary bg-white color-black sorterinput listfilter mr-2" placeholder="filtern nach ..." id="einsatzusersort" data-target="#inEinsatz .members li" type="text"><i title="Löschen" class="sync material-icons">backspace</i>
					<div class="clearfix"></div>
			...
			<div id="inPause" class="groupbox col-12 col-lg-4 p-2">
				<div class="bg-lightgray p-2">
					<h4 class="ui-widget-header btn bg-red color-white w-100">In Pause</h4>
					<div class="clearfix"></div>
					<ul class="members list-group w-100">
						<?php
