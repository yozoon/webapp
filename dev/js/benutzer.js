...
// Mitglied aktivieren und in personen_im_einsatz speichern
	...
	let Mitglied_aus_Einsatz_nehmen = function(target,item) {
		...
		if(target == "#members"){
			...
			item.removeClass("btn-outline-success")
				...
				.fadeIn()
				.find("i").remove();
			item.prepend('<i title="Mitglied editieren" class="edituser material-icons">mode_edit</i>').append('<i class="moveright material-icons ml-auto">arrow_forward</i>').appendTo($("#collapse"+oid)).fadeIn();
			let json_nodes = {
				status: 11,
				gruppe:"",
	...
	let Mitglied_bearbeiten = function(that){
		...
		let userPause = that.parent().attr("data-pause");
		let newTyp,newPause;
		
		$(".changeuser").modal("show").find("h5").html(userID+" "+userName);
		$("#changeUser").attr('data-userID',userID);
		$("#changeUser .usertyp").val(userTyp).prop('selected', true);
		$("#changeUser .userpause").val(userPause*60).prop('selected', true);
		
		$('#changeUser').on('change','.usertyp',function(){
			if($("#changeUser .usertyp").val() != "HF"){
				$("#changeUser .userpause").val("0").prop('selected', true);
			}else{
				$("#changeUser .userpause").val("3600").prop('selected', true);
			}
			newTyp = $("#changeUser .usertyp option:selected").val();
			newPause = $("#changeUser .userpause option:selected").val();
		});
