...
if($p != false || $p_post != false){
	...
	$tokencheck = false;
	$tokenexpired = true;
	$select = $p != false ? $db->prepare("SELECT  COUNT(*) from user WHERE pwdresetkey LIKE '".$p."'") : $db->prepare("SELECT  COUNT(*) from user WHERE pwdresetkey LIKE '".$p_post."'");
	if($select->execute()) {
	...
	<div class="rounded m-4" style="background-color:rgba(255,255,255,0.5);">
		...
		switch($step){
			...
			?>
				...
				<?php
				$db_mitglieder = $db->prepare("SELECT * FROM user WHERE pwdresetkey = '".$p."'");
		...
		<?php
			...
			$db_mitglieder = $db->prepare("SELECT * FROM user WHERE OID = '".$oid4reset."' AND pwdresetkey = '".$p_post."'");
			$db_mitglieder->execute($db_mitglieder->errorInfo());
			$email_t = $uid_t = "";
			while ($res_mg = $db_mitglieder->fetch(PDO::FETCH_ASSOC)){
				$data_user_json = json_decode(substr(string_decrypt($res_mg['data']), 1, -1));
				...
				if($pwdresetkey == $p_post){
					...
					$update = $db->prepare("UPDATE user SET `data`='".$encrypted."',`pwd`='".$newpassword."',`pwdresetkey`='' WHERE `UID`='".$uid_t."' AND OID = '".$oid4reset."'");
		...
		<script src="v5/js/jquery-2.2.3.min.js"></script>
